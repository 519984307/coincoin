...
*Setup instructions for a Gitian build of GeekCash using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the GeekCash
...
- [Building GeekCash](#building-geek-cash)
...
Clone the git repositories for GeekCash and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/blazegeek/blazegeekcash
...
in the GeekCash repository under 'Fetch and create inputs' to install sources which require
...
To build GeekCash (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the GeekCash repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/geekcash/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/blazegeek/blazegeekcash
...
For example:
```bash
URL=https://github.com/crowning-/geekcash.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit geekcash=${COMMIT} --url geekcash=${URL} ../geekcash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit geekcash=${COMMIT} --url geekcash=${URL} ../geekcash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit geekcash=${COMMIT} --url geekcash=${URL} ../geekcash/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the geekcash git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../geekcash/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/geekcash
SIGPATH=/some/root/path/geekcash-detached-sigs

./bin/gbuild --url geekcash=${BTCPATH},signature=${SIGPATH} ../geekcash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/geekcash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/geekcash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/geekcash-osx-build.assert
```
...
[geekcash/gitian.sigs](https://github.com/geekcashpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
