...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete CoinCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no CoinCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old CoinCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected CoinCoin 0.7 datadir."
    ;;
  3)
    echo "Detected CoinCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected CoinCoin 0.8 datadir."
    ;;
esac
